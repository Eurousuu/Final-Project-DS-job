...
df = pd.read_csv("C:\\Users\\廖宇嘉\\OneDrive\\MISY225\\Cleaned_DS_Jobs.csv")
...
if option == 'Combination':
    ...
    st.header(' Age ') # 添加标题
    draw_combine3()
    st.write("This scatter plot mainly reflects the approximate relationship between company age and job type.")
    st.write("It is obvious that these data jobs are all important for emerging companies, which are mainly concentrated in companies under the age of 50.")
    st.write("Among them, data scientist shows the most obvious performance. The younger the company, the newer the company, and the higher their professional demand for data scientists. ")

    button_click = st.button(':sparkles: Conclusion')
    if button_click:
        st.write("That's to say, in today's rapidly developing society of technology and information, data analysis has become an indispensable task for firms")
